def GeneralListFunction(websiteurl,sitetype):
    ...
    ###################################################################################################
    if(sitetype):
        tofindregex="Aqui voce insere o regex necessario para achar cada carta no teu site"
    else:
        tofindregex=r"""class='deck-qty'>([1-9][0-9]*?)&.*?&card=(.*?)">"""
    Cards=list(set(findall(tofindregex,page)))
